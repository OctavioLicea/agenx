...
>
> Última actualización: Sesión 16, 15 de julio 2026 (cierre de
> duplicados sin limpiar, code-splitting del bundle, homologación de
> búsqueda por rol/empresa, fix de scroll en buscadores, limpieza de
> `.git/`, exportar vCard, escáner de documentos con cámara; bug de
> sync del sandbox reapareció al verificar el build, sin forzar nada —
> ver sección de Bundle abajo).
...
- [ ] **Bundle de producción pesa 716 kB JS (gzip)** (Sesión 16, en progreso): se aplicó `React.lazy()` + `Suspense` a `PropiedadForm`, `ContactoForm`, `PerfilForm` (en `App.jsx`, antes se cargaban siempre aunque `vista` no los necesitara) y a `ExportaFicha` (dentro de `PropiedadForm.jsx`, saca `@react-pdf/renderer` — el paquete más pesado — del bundle inicial, solo carga al exportar un PDF). **Falta verificar el tamaño real con build** — el bug de sync del sandbox (ver incidente de Sesión 15) volvió a aparecer esta sesión mientras se intentaba correr el build de verificación (bash mostró `App.jsx` con contenido de un día antes, sin los cambios de hoy) y no se forzó nada para no repetir la corrupción de Sesión 15. Pendiente confirmar en la próxima sesión (o desde la máquina de Okta con `npm run build`) que sigue compilando bien y cuánto bajó el bundle.
- [ ] **Push a `origin/main` pendiente** (Sesión 15, sigue igual en Sesión 16) — commits locales sin subir (ahora incluyen también los de hoy). El sandbox de Cowork no tiene credenciales de GitHub por política (confirmado explícitamente, ver "Decisiones tomadas" en `docs/bitacora/2026-07-15.md`) y no se van a aceptar tokens/contraseñas para resolverlo. Sigue requiriendo que Okta corra `git pull` + `git push origin main` desde su máquina.
