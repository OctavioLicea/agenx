...
>
> Última actualización: Sesión 15, 15 de julio 2026 (fix de notas en
> ficha de contacto, incidente de corrupción por bug de sync del
> sandbox, build verificado en Cowork por primera vez).
...
- [ ] **Decidir si limpiar los duplicados/contactos sin teléfono** antes de que Nydia empiece a usarlos en serio (334 grupos duplicados, 100 sin teléfono — ver hallazgo arriba). No es urgente, pero conviene resolverlo antes de que el volumen de uso real dificulte identificar cuáles son los originales.
...
- [ ] **Push a `origin/main` pendiente** (Sesión 15) — los commits de hoy (`ef039b8`, `67c7471`, `9104666`, `0a3128b`) quedaron hechos en el repo local montado en Cowork, pero el sandbox no tiene credenciales de GitHub para hacer push. Falta que Okta corra `git pull` (trae los commits, ya están en el mismo disco montado así que no debería haber conflicto) y `git push origin main` desde su máquina.
- [ ] **Limpieza de archivos sueltos en `.git/`** (arrastrado desde Sesión 12, cuantificado en Sesión 15) — 9 archivos `*.lock.old*`/`*.stale_*` + 127 `tmp_obj_*` en `.git/objects/`. No afectan el repo (basura inerte de commits interrumpidos por el bug de locks del sandbox), pero Cowork no puede borrarlos (`Operation not permitted` incluso con `mv`). Requiere que Okta los borre a mano desde Windows (mostrar archivos ocultos, o `del /S *.lock.old* objects\*\tmp_obj_*` desde la carpeta `.git`).
